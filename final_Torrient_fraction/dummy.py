...
def diversity_score(i_determinant,dissim_mat):
    # import pdb;pdb.set_trace()
    diversity = torch.matmul(i_determinant,dissim_mat)
    print(diversity)
    print(diversity.shape)
    max_div = sorted(zip(diversity), reverse=True)[:3]
    print(max_div)
    # sorted(range(len(a)), key=lambda i: a[i])[-2:]
    # max_index= sorted(range(len(max_div)), key=lambda i: max_div[i])[-3:]
    # max_index = sorted(range(len(max_div)), key=lambda i: max_div[i], reverse=True)[:3]
    # print(max_index)
    # max_index = values, indices = torch.max(diversity, 0)
    max_index = torch.topk(diversity, 3)
    print(max_index)

    print("det&dissim mul vales printed here ")
...
def main():
    ...
    find_diversity = diversity_score(i_determinant,dissim_mat)
